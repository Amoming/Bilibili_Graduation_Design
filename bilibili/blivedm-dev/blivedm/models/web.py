...
class DanmakuMessage:
    ...
    def from_command(cls, info: list):
        ...
        extra_str: str =info[0][15]['extra']
        extra_list = extra_str.split(',')
        data_str = ','.join(extra_list)
        extra_data = json.loads(data_str)
        datatime_format = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(info[9]['ts']))
        time_format = time.strftime('%H:%M:%S', time.localtime(info[9]['ts']))
